...
impl ContractCombinator for ThenCombinator {
    ...
    fn acquire(&mut self, time: u32, storage: &mut Storage) {
        ...
        sub_combinator.acquire(time, storage);
...
mod tests {
    ...
    fn acquiring_and_updating_returns_correct_value() {
        ...
        combinator.acquire(2, &mut Storage::new());
        let value = combinator.update(2, &mut Storage::new());
    ...
    fn updating_before_acquisition_time_does_nothing() {
        ...
        let value = combinator.update(1, &mut Storage::new());
        let combinator_details = combinator.get_combinator_details();
